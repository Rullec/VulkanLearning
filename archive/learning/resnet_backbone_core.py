...
class ResnetBackboneFC(nn.Module):
    def __init__(self, version, input_size, output_size):
        super(ResnetBackboneFC, self).__init__()
        if version == "50":
            base_class = ResNet50
        ...
        self.base = base_class(inplanes=input_size)
        self.relu = nn.ReLU(inplace=True)
        ...
        if load_pretrained_weights is True:
            ...
            # if True:
            #     self_len = len(self_model_keys)
            #     pretrained_len = len(pretrained_model_keys)
            #     print(f"self model key len {self_len}")
            #     print(f"pretrained model key len {pretrained_len}")
            #     for i in range(self_len):
            #         self_param_name = self_model_keys[i]
            #         pretrained_param_name = pretrained_model_keys[i]
            #         print(
            #             f"self {self_param_name}, pretrained {pretrained_param_name}"
            #         )
            # print(f"self model key  {(self_model_keys)}")
            # print(f"pretrained model key  {(pretrained_model_keys)}")
...
if __name__ == "__main__":
    import numpy as np
    inplanes = 4
    # net = ResnetBackboneFC("50", input_size=inplanes, output_size=3)
    net = ResnetBackboneFC("18", input_size=inplanes, output_size=3)
    net.show_parameters()
    # print(net)
    data = np.ones([1, inplanes, 512, 512])
